...
const QUOTE_STANDARD_HISTORY=[
  {
    version:'2026/08/29-V46',
    note:'沿用V45全部內容，只新增31–35層標準：31=361Y、32=354Y、33=347Y、34=340Y、35=333Y；延續每層較前一層下降2%的現場標準 / Giữ nguyên toàn bộ V45, chỉ thêm chuẩn lớp 31–35: 361Y, 354Y, 347Y, 340Y, 333Y; tiếp tục chuẩn hiện trường giảm 2% so với lớp trước.',
    layers:{
      WATER:[{min:0,max:0,tables12:2.5},{min:1,max:1,tables12:2.5},{min:2,max:2,tables12:2.5},{min:3,max:3,tables12:2.5},{min:4,max:4,tables12:2.5},{min:5,max:5,tables12:2.375},{min:6,max:6,tables12:2.25},{min:7,max:7,tables12:2.125},{min:8,max:8,tables12:2},{min:9,max:9,tables12:1.875},{min:10,max:10,tables12:1.75},{min:11,max:11,tables12:1.625},{min:12,max:12,tables12:1.5},{min:13,max:13,tables12:1.375},{min:14,max:14,tables12:1.25},{min:15,max:15,tables12:1.125},{min:16,max:16,tables12:1},{min:17,max:17,tables12:0.875},{min:18,max:18,tables12:0.75},{min:19,max:19,tables12:0.625},{min:20,max:20,tables12:0.5},{min:21,max:21,tables12:0.49},{min:22,max:22,tables12:0.4802},{min:23,max:23,tables12:0.470596},{min:24,max:24,tables12:0.46118408},{min:25,max:25,tables12:0.4519603984},{min:26,max:26,tables12:0.442921190432},{min:27,max:27,tables12:0.434062766623},{min:28,max:28,tables12:0.425381511291},{min:29,max:29,tables12:0.416873881065},{min:30,max:30,tables12:0.408536403444},{min:31,max:31,tables12:0.401111111111},{min:32,max:32,tables12:0.393333333333},{min:33,max:33,tables12:0.385555555556},{min:34,max:34,tables12:0.377777777778},{min:35,max:35,tables12:0.37}],
      SILICONE:[{min:0,max:0,tables12:2.5},{min:1,max:1,tables12:2.5},{min:2,max:2,tables12:2.5},{min:3,max:3,tables12:2.5},{min:4,max:4,tables12:2.5},{min:5,max:5,tables12:2.375},{min:6,max:6,tables12:2.25},{min:7,max:7,tables12:2.125},{min:8,max:8,tables12:2},{min:9,max:9,tables12:1.875},{min:10,max:10,tables12:1.75},{min:11,max:11,tables12:1.625},{min:12,max:12,tables12:1.5},{min:13,max:13,tables12:1.375},{min:14,max:14,tables12:1.25},{min:15,max:15,tables12:1.125},{min:16,max:16,tables12:1},{min:17,max:17,tables12:0.875},{min:18,max:18,tables12:0.75},{min:19,max:19,tables12:0.625},{min:20,max:20,tables12:0.5},{min:21,max:21,tables12:0.49},{min:22,max:22,tables12:0.4802},{min:23,max:23,tables12:0.470596},{min:24,max:24,tables12:0.46118408},{min:25,max:25,tables12:0.4519603984},{min:26,max:26,tables12:0.442921190432},{min:27,max:27,tables12:0.434062766623},{min:28,max:28,tables12:0.425381511291},{min:29,max:29,tables12:0.416873881065},{min:30,max:30,tables12:0.408536403444},{min:31,max:31,tables12:0.401111111111},{min:32,max:32,tables12:0.393333333333},{min:33,max:33,tables12:0.385555555556},{min:34,max:34,tables12:0.377777777778},{min:35,max:35,tables12:0.37}]
    },
    strips:{
      HAND:[{width:6,strips:38},{width:8,strips:30},{width:9,strips:28},{width:10,strips:26},{width:11,strips:24},{width:12,strips:22},{width:13,strips:20},{width:14,strips:20},{width:15,strips:18},{width:16,strips:17},{width:17,strips:16},{width:18,strips:15},{width:20,strips:14},{width:21,strips:13},{width:22,strips:13},{width:24,strips:12},{width:25,strips:11},{width:28,strips:10},{width:30,strips:10},{width:36,strips:8},{width:40,strips:8},{width:45,strips:7},{width:86,strips:3}],
    ...
    sides:2,
    special:{
      SHOELACE:{qtyPerTable:1500,unit:'PAIR',workers:4,layHours:8,temp:false},
      PIECE:{qtyPerTable:5000,unit:'PC',workers:4,layHours:4,temp:false}
    }
  },
  {
    version:'2026/08/21',
...
const QUOTE_DIP_ALLOC_HOURS=4;
function quoteDipFindHeadQc(type,bodyQc){
...
function quoteMachineApplyStripStandard(force=false){
  ...
  if(r&&(force||!quoteMachineStripsManual)){
    ...
    $('quoteMachineStripsHint').textContent=`依機印規格 ${fmt(qc,2)}MM 標準，自動帶入 ${fmt(r.strips,0)} 條 / Theo chuẩn QC In máy ${fmt(qc,2)}MM, tự động lấy ${fmt(r.strips,0)} dây`;
...
function quoteSetMachineUnit(unit){
  ...
  if($('quoteMachinePatternsField'))$('quoteMachinePatternsField').classList.toggle('hidden',unit!=='PC');
  if($('quoteMachineLengthZh'))$('quoteMachineLengthZh').textContent=unit==='PC'?'每 PC 長度（mm）':'網版圖案總長度（MM）';
  if($('quoteMachineLengthVi'))$('quoteMachineLengthVi').textContent=unit==='PC'?'Độ dài mỗi PC (mm)':'Tổng độ dài hình trên khuôn (MM)';
  if($('quoteMachinePullHint'))$('quoteMachinePullHint').textContent=unit==='PC'?'每 PC 長度 × 每行圖案數 × 10，自動計算，請勿修改 / Độ dài mỗi PC × số hình trong 1 hàng × 10, tự động tính, không điều chỉnh':'網版圖案總長度 × 10，自動計算，請勿修改 / Tổng độ dài hình trên khuôn × 10, tự động tính, không điều chỉnh';
  calcQuoteCapacity();
}
...
function quoteLayerSourceText(layers){
  if(!Number.isFinite(Number(layers)))return '';
  const n=Number(layers);
  if(n<=4)return '0–4層依現場固定標準2.5桌/12H / Lớp 0–4 theo chuẩn hiện trường cố định 2,5 bàn/12H';
  if(n<=20)return '5–20層依現場逐層標準表 / Lớp 5–20 theo bảng tiêu chuẩn hiện trường từng lớp';
  if(n<=30){const precise=.5*Math.pow(.98,n-20);return `21–30層延伸：20層0.5桌 × 98%^${fmt(n-20,0)} = ${fmt(precise,4)}桌/12H / Mở rộng lớp 21–30: lớp 20 = 0,5 bàn × 98%^${fmt(n-20,0)} = ${fmt(precise,4)} bàn/12H`;}
  if(n<=35){const yardMap={31:361,32:354,33:347,34:340,35:333},prevMap={31:368,32:361,33:354,34:347,35:340};return `${n}層延續標準：前一層 ${prevMap[n]}Y × 98% ≈ ${yardMap[n]}Y（依現場表） = ${fmt(yardMap[n]/900,4)}桌/12H / Lớp ${n}: lớp trước ${prevMap[n]}Y × 98% ≈ ${yardMap[n]}Y (theo bảng hiện trường) = ${fmt(yardMap[n]/900,4)} bàn/12H`;}
  return '目前層數標準只建立到35層 / Tiêu chuẩn số lớp hiện chỉ thiết lập đến lớp 35';
}
...
function quoteEstimateStrips(method,width,std=QUOTE_STANDARD){
  ...
  // K3 新標準只有表內列出的規格才是正式值；空白規格不自行插值，避免把估算誤當標準。
  if(method==='K3')return null;
...
function quoteApplyStripSuggestion(force=false){
  ...
  if(r&&(force||quoteStripsAuto||!Number($('quoteStrips').value))){
    ...
    $('quoteStripsHint').textContent=r.estimated?`規格 ${fmt(width,2)}MM 非表內正式值，本次估算單邊 ${fmt(r.strips,0)} 條 / QC ${fmt(width,2)}MM không có trong bảng chính thức, lần này ước tính ${fmt(r.strips,0)} sợi/1 bên`:`依規格 ${fmt(width,2)}MM 正式標準，自動帶入單邊 ${fmt(r.strips,0)} 條 / Theo chuẩn chính thức QC ${fmt(width,2)}MM, tự động lấy ${fmt(r.strips,0)} sợi/1 bên`;
...
function setQuoteMachineStroke(stroke){
  quoteMachineStroke=stroke;
  document.querySelectorAll('.quote-machine-stroke-btn').forEach(btn=>btn.classList.toggle('active',Number(btn.dataset.stroke)===stroke));
  calcQuoteCapacity();
}
function updateQuoteTableLength(){
  const m=$('quoteMethod').value;
  const isMachine=m==='MACHINE',isDip=m==='DIP';
  ['quoteLayoutField','quoteInkField','quoteWidthField','quoteStripsField','quotePcLengthField','quoteLayersField','quoteTableLengthField'].forEach(id=>{if($(id))$(id).classList.toggle('hidden',isMachine||isDip);});
  if($('quoteMachineFields'))$('quoteMachineFields').classList.toggle('hidden',!isMachine);
  if($('quoteDipFields'))$('quoteDipFields').classList.toggle('hidden',!isDip);
  if($('quoteAlert'))$('quoteAlert').classList.toggle('hidden',isMachine||isDip);
  if(isDip){
  ...
  ['quoteDipSingleHeadResult','quoteDipPcInkResult','quoteDipWasteResult','quoteDipTotalInkResult'].forEach(id=>{if($(id))$(id).classList.add('hidden');});
  if(isMachine){
    if($('quoteUnitToggle'))$('quoteUnitToggle').classList.add('hidden');
...
function updateQuoteLayoutUI(){
  const method=$('quoteMethod').value;
  if(method==='DIP'){quoteDipUpdateDepthFields();return;}
  if(method==='MACHINE'){calcQuoteCapacity();return;}
  ...
  if(isSpecial&&sp){
    ...
    if(isPiece&&method==='HAND'){$('quoteSpecialQty').textContent='依排版計算 / Tính theo bố trí';$('quoteSpecialWorkers').textContent='25M × 2邊 / 2 bên';$('quoteSpecialLayHours').textContent='間隔6MM / Cách 6MM';}
    else if(isPiece&&method==='K3'){$('quoteSpecialQty').textContent='44格 × 每格PC / 44 ô × PC/ô';$('quoteSpecialWorkers').textContent='固定44格 / Cố định 44 ô';$('quoteSpecialLayHours').textContent='依層數產能 / Theo số lớp';}
    else{$('quoteSpecialQty').textContent=`${fmt(sp.qtyPerTable,0)} ${sp.unit==='PAIR'?'雙 / đôi':'PC'}`;$('quoteSpecialWorkers').textContent=`${fmt(sp.workers,0)} 人 / người`;$('quoteSpecialLayHours').textContent=`${fmt(sp.layHours,2)} H / giờ`;}
  }
  if(!isSpecial){$('quotePcLengthField').classList.toggle('hidden',quoteUnit!=='PC');if($('quoteUnitToggle'))$('quoteUnitToggle').classList.remove('hidden');}
  calcQuoteCapacity();
}
function calcQuoteCapacity(){
  ...
  if(method==='MACHINE'){
    const patterns=quoteMachineUnit==='PC'?Math.max(1,Number($('quoteMachinePatterns')?.value)||1):1,lengthMm=Number($('quoteMachineLength').value),pullSpeed=200,qc=Number($('quoteMachineQc').value);
    ...
    const yardsPerCycle=(lengthMm>0&&strips>0)?lengthMm/914.4*strips:0;
    const pcPerCycle=(patterns>0&&strips>0)?patterns*strips:0;
    const perHourY=yardsPerCycle*framesHour,perHourPc=pcPerCycle*framesHour;
    const perHour=quoteMachineUnit==='PC'?perHourPc:perHourY,cap8=perHour*8,cap12=perHour*12;
    $('quoteWhatYouAreQuoting').textContent=`目前：機印｜規格 ${qc>0?fmt(qc,2):'—'}mm｜${strips>0?fmt(strips,0):'—'}條｜${quoteMachineStroke}刀｜${quoteMachineUnit} / Hiện tại: In máy｜QC ${qc>0?fmt(qc,2):'—'}mm｜${strips>0?fmt(strips,0):'—'} dây｜${quoteMachineStroke} gạt｜${quoteMachineUnit}`;
    $('quoteMachineHour').textContent=perHour>0?`${fmt(perHour,quoteMachineUnit==='PC'?0:2)} ${quoteMachineUnit}/H`:'—';
    $('quoteMachine12').textContent=cap12>0?`${fmt(cap12,quoteMachineUnit==='PC'?0:2)} ${quoteMachineUnit}`:'—';
    $('quoteCapacity8').textContent=cap8>0?`${fmt(cap8,quoteMachineUnit==='PC'?0:2)} ${quoteMachineUnit}`:'—';
    $('quoteCapacity8Alt').textContent=perHour>0?`${fmt(strips,0)} 條同時印刷 / In cùng lúc ${fmt(strips,0)} sợi`:'請輸入規格、長度與印刷條數 / Vui lòng nhập QC, độ dài và số dây in';
    if(patterns>0&&lengthMm>0&&strips>0&&perHour>0){
      if(quoteMachineUnit==='PC')$('quoteFormula').textContent=`PC：每循環 = ${fmt(patterns,0)} 圖案 × ${fmt(strips,0)}條 = ${fmt(pcPerCycle,0)} PC；建議拉帶長度 = ${fmt(lengthMm,2)}mm × ${fmt(patterns,0)}圖案 × 10 = ${fmt(pullLength,0)}；速度200、${quoteMachineStroke}刀 → ${fmt(perHourPc,0)} PC/H，8H ${fmt(cap8,0)} PC。 / PC: mỗi chu kỳ = ${fmt(patterns,0)} hình × ${fmt(strips,0)} dây = ${fmt(pcPerCycle,0)} PC; chiều dài kéo đề nghị = ${fmt(lengthMm,2)}mm × ${fmt(patterns,0)} hình × 10 = ${fmt(pullLength,0)}; tốc độ 200, ${quoteMachineStroke} gạt → ${fmt(perHourPc,0)} PC/H, 8H ${fmt(cap8,0)} PC.`;
      else $('quoteFormula').textContent=`Y：網版圖案總長度作為一循環；每循環碼數 = ${fmt(lengthMm,2)} ÷ 914.4 × ${fmt(strips,0)}條 = ${fmt(yardsPerCycle,3)} Y；速度200、${quoteMachineStroke}刀 → ${fmt(perHourY,2)} Y/H，8H ${fmt(cap8,2)} Y。 / Y: tổng độ dài hình trên khuôn là 1 chu kỳ; yard/chu kỳ = độ dài ÷ 914.4 × số dây; tốc độ 200, ${quoteMachineStroke} gạt.`;
    } else $('quoteFormula').textContent='請輸入規格、長度與印刷條數。PC請填每PC長度；Y請填網版圖案總長度。 / Nhập QC, độ dài và số dây in. PC nhập độ dài mỗi PC; Y nhập tổng độ dài hình trên khuôn.';
    return;
  }
  if(layout!=='NORMAL'){
    ...
    if(layout==='PIECE'){
      const tables12=rule?Number(rule.tables12):0,tables8=tables12*8/12;let qty=0,detail='';
      if(method==='HAND'){
        const pcPerRow=Number($('quoteHandPiecePcPerRow')?.value),usableMm=25000,gapMm=6;
        const rows=pieceLen>0?Math.floor((usableMm+gapMm)/(pieceLen+gapMm)):0;
        qty=(rows>0&&pcPerRow>0)?rows*pcPerRow*2:0;
        detail=rows>0&&pcPerRow>0?`25M = 25,000MM；每排占用 ${fmt(pieceLen,2)} + 6 = ${fmt(pieceLen+6,2)}MM；單邊 ${rows}排 × ${fmt(pcPerRow,0)}PC × 2邊 = ${fmt(qty,0)} PC/桌 / 25M = 25.000MM; mỗi hàng ${fmt(pieceLen,2)} + 6 = ${fmt(pieceLen+6,2)}MM; 1 bên ${rows} hàng × ${fmt(pcPerRow,0)}PC × 2 bên = ${fmt(qty,0)} PC/bàn`:'請輸入每 PC 長度與每排可排 PC / Vui lòng nhập độ dài mỗi PC và số PC mỗi hàng';
      }else if(method==='K3'){
        const pcPerCell=Number($('quoteK3PiecePcPerCell')?.value);qty=pcPerCell>0?44*pcPerCell:0;
        detail=pcPerCell>0?`固定44大格 × ${fmt(pcPerCell,0)} PC/格 = ${fmt(qty,0)} PC/桌 / Cố định 44 ô lớn × ${fmt(pcPerCell,0)} PC/ô = ${fmt(qty,0)} PC/bàn`:'請輸入每大格可排 PC / Vui lòng nhập số PC mỗi ô lớn';
      }
      const cap8=qty*tables8,cap12=qty*tables12;
      $('quoteWhatYouAreQuoting').textContent=`目前：${quoteMethodName(method)}｜單片｜${inkLabel}${Number.isFinite(layers)?'｜'+fmt(layers,0)+'層':''} / Hiện tại: ${quoteMethodName(method)}｜Miếng rời｜${inkLabel}${Number.isFinite(layers)?'｜'+fmt(layers,0)+' lớp':''}`;
      $('quoteTotalStrips').textContent=method==='HAND'?'25M單邊 × 2邊 / 25M mỗi bên × 2':'44大格 / 44 ô lớn';
      $('quotePerTable').textContent=qty>0?`${fmt(qty,0)} PC`:'—';$('quoteTables12').textContent=tables12>0?`${fmt(tables12,3)} 桌 / bàn`:'—';$('quoteTables8').textContent=tables8>0?`${fmt(tables8,3)} 桌 / bàn`:'—';$('quoteCapacity12').textContent=cap12>0?`${fmt(cap12,0)} PC`:'—';$('quoteCapacity8').textContent=cap8>0?`${fmt(cap8,0)} PC`:'—';$('quoteCapacity8Alt').textContent=qty>0&&tables8>0?`每桌 ${fmt(qty,0)} PC × 8H ${fmt(tables8,3)}桌 / ${fmt(qty,0)} PC/bàn × ${fmt(tables8,3)} bàn/8H`:'請完成排版參數與層數 / Hoàn tất thông số bố trí và số lớp';
      if(!Number.isFinite(layers)||!rule)$('quoteFormula').textContent='請輸入有效層數；8H桌數依「層數產能標準」換算。 / Vui lòng nhập số lớp hợp lệ; số bàn 8H lấy theo tiêu chuẩn năng suất số lớp.';
      else $('quoteFormula').textContent=`${detail}；層數標準來源：${quoteLayerSourceText(layers)}；${fmt(layers,0)}層 = ${fmt(tables12,4)}桌/12H → × 8/12 = ${fmt(tables8,4)}桌/8H；8H產能 = 每桌${fmt(qty,0)} PC × ${fmt(tables8,4)}桌 = ${fmt(cap8,0)} PC。 / ${detail}; nguồn chuẩn số lớp: ${quoteLayerSourceText(layers)}; ${fmt(layers,0)} lớp = ${fmt(tables12,4)} bàn/12H → × 8/12 = ${fmt(tables8,4)} bàn/8H; năng suất 8H = ${fmt(qty,0)} PC/bàn × ${fmt(tables8,4)} = ${fmt(cap8,0)} PC.`;
      return;
    }
    const qty=sp?Number(sp.qtyPerTable):0,layHours=sp?Number(sp.layHours):0,workers=sp?Number(sp.workers):0,printHours=Number.isFinite(layers)&&layers>0?layers:0,totalHours=layHours+printHours,cap8=totalHours>0?qty/totalHours*8:0,cap12=totalHours>0?qty/totalHours*12:0;
    ...
    $('quoteTotalStrips').textContent='特殊排料 / Xếp liệu đặc biệt';$('quotePerTable').textContent=qty>0?`${fmt(qty,0)} ${unit}`:'—';$('quoteTables12').textContent=printHours>0?`${fmt(printHours,2)} H / 桌印刷 / giờ in mỗi bàn`:'—';$('quoteTables8').textContent=totalHours>0?`${fmt(8/totalHours,2)} 桌 / bàn`:'—';$('quoteTotalHours').textContent=totalHours>0?`${fmt(totalHours,2)} H / giờ（${fmt(layHours,2)}+${fmt(printHours,2)}）`:'—';$('quoteCapacity12').textContent=cap12>0?`${fmt(cap12,0)} ${unit}`:'—';$('quoteCapacity8').textContent=cap8>0?`${fmt(cap8,0)} ${unit}`:'—';$('quoteCapacity8Alt').textContent=`排料 ${fmt(workers,0)}人 × ${fmt(layHours,2)}H = ${fmt(workers*layHours,0)} 人時/桌 / Xếp liệu ${fmt(workers,0)} người × ${fmt(layHours,2)}H = ${fmt(workers*layHours,0)} giờ-người/bàn`;
    $('quoteFormula').textContent=(!sp||!Number.isFinite(layers)||!(layers>0))?'請輸入層數並確認特殊排料標準 / Vui lòng nhập số lớp và kiểm tra tiêu chuẩn xếp liệu đặc biệt':`每桌 ${fmt(qty,0)} ${unit}；排料 ${fmt(layHours,2)}H + ${fmt(layers,0)}層印刷 ${fmt(printHours,2)}H = ${fmt(totalHours,2)}H/桌；8H產能 ${fmt(cap8,0)} ${unit}。 / 1 bàn ${fmt(qty,0)} ${unit}; xếp ${fmt(layHours,2)}H + in ${fmt(printHours,2)}H = ${fmt(totalHours,2)}H; năng suất 8H ${fmt(cap8,0)}.`;
    return;
  }
  ...
  $('quoteWhatYouAreQuoting').textContent=`目前：${quoteMethodName(method)}｜一般長帶｜${inkLabel}｜${quoteUnit}${width>0?'｜規格 '+fmt(width,2)+' mm':''} / Hiện tại: ${quoteMethodName(method)}｜Dây dài｜${inkLabel}｜${quoteUnit}${width>0?'｜QC '+fmt(width,2)+' mm':''}`;
  ...
  else $('quoteFormula').textContent=`一般長帶：單邊 ${fmt(oneSide,0)}條（依QC標準） × ${sides}邊 × 桌長 ${fmt(tableY,2)}Y = ${fmt(perTableY,2)}Y/桌；層數標準來源：${quoteLayerSourceText(layers)}；${fmt(layers,0)}層 = ${fmt(rule.tables12,4)}桌/12H → × 8/12 = ${fmt(tables8,4)}桌/8H。 / Dây dài: ${fmt(oneSide,0)} sợi một bên (theo chuẩn QC) × ${sides} bên × ${fmt(tableY,2)}Y = ${fmt(perTableY,2)}Y/bàn; nguồn chuẩn số lớp: ${quoteLayerSourceText(layers)}; ${fmt(layers,0)} lớp = ${fmt(rule.tables12,4)} bàn/12H → × 8/12 = ${fmt(tables8,4)} bàn/8H.`;
}
function quoteLayerTable(std){
  ...
  const rows=keys.map(l=>{const w=valueAt(water,l),si=valueAt(sil,l),exampleY=w!=null?900*w:null;const source=l<=4?'現場標準 2.5桌 / Chuẩn hiện trường 2,5 bàn':l<=20?'依現場逐層表 / Theo bảng hiện trường từng lớp':'前一層 × 98% / Lớp trước × 98%';return `<tr><td>${l}</td><td>${w==null?'—':fmt(w,4)}</td><td>${si==null?'—':fmt(si,4)}</td><td>${exampleY==null?'—':fmt(exampleY,0)+' Y'}</td><td>${source}</td></tr>`;}).join('');
  return `<div class="quote-standard-section"><b>層數產能 / Năng suất theo số lớp</b><div class="quote-standard-explain"><b>數字怎麼來 / Cách hình thành số liệu：</b><br>0–4層＝2.5桌/人/12H。5–20層依現場逐層標準下降；例如15mm、單邊18條、25Y、2邊時，1桌＝18 × 25Y × 2＝900Y。20層＝0.5桌＝450Y/12H。<br><b>21–30層延伸規則：</b>從20層開始，每增加1層，產能＝前一層精確值 × 98%（下降2%）；計算保留精確值，畫面Y再四捨五入。例如21層＝450 × 98%＝441Y；22層＝441 × 98%＝432.18Y ≈ 432Y。<br><b>31–35層新增現場標準：</b>沿用每層較前一層下降2%的原則，依你提供的正式表固定為 31層361Y、32層354Y、33層347Y、34層340Y、35層333Y；桌數以900Y/桌換算。<br>0–4 lớp = 2,5 bàn/người/12H. Lớp 5–20 theo bảng tiêu chuẩn hiện trường. Ví dụ 15mm, 18 sợi một bên, 25Y, 2 bên: 1 bàn = 18 × 25Y × 2 = 900Y. Lớp 20 = 0,5 bàn = 450Y/12H. <b>Lớp 21–30:</b> mỗi tăng 1 lớp, năng suất = giá trị chính xác của lớp trước × 98%; Yard hiển thị được làm tròn. <b>Lớp 31–35:</b> theo bảng hiện trường mới: 361Y, 354Y, 347Y, 340Y, 333Y; số bàn được quy đổi theo 900Y/bàn.</div><div class="table-scroll"><table class="quote-settings-table"><thead><tr><th>總層數<br>Tổng số lớp</th><th>水性 1人/12H桌數<br>Mực nước: Bàn/người/12H</th><th>SILICONE 1人/12H桌數<br>Bàn/người/12H</th><th>900Y/桌示例產能<br>Ví dụ Y/12H (900Y/bàn)</th><th>數據來源<br>Nguồn số liệu</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
}
function quoteStripTable(title,rows,isMachine=false){
  ...
  const second=isMachine?'印刷條數<br>Số dây in':'單邊排帶條數<br>Tổng số sợi 1 bên bàn';
  return `<div class="quote-standard-section"><b>${title}</b><table class="quote-settings-table"><thead><tr><th>帶身規格 (MM)<br>QC (MM)</th><th>${second}</th></tr></thead><tbody>${body}</tbody></table></div>`;
}
function quoteDipHeadQcTable(std){
  const rows=std.dipHeadQc||[];
  const body=rows.length?rows.map(r=>`<tr><td>${quoteEscape(r.type)}</td><td>${fmt(r.bodyQc,2)}</td><td>${fmt(r.headQc,2)}</td></tr>`).join(''):`<tr><td colspan="3">尚未建立完整標準 / Chưa có tiêu chuẩn đầy đủ</td></tr>`;
  return `<div class="quote-standard-section"><b>浸染束頭規格標準 / Tiêu chuẩn QC đầu dây nhúng sơn</b><div class="table-scroll"><table class="quote-settings-table"><thead><tr><th>帶型<br>Loại dây</th><th>帶身QC<br>QC thân dây</th><th>束頭QC<br>QC đầu dây</th></tr></thead><tbody>${body}</tbody></table></div><div class="quote-standard-explain">目前已建立的正式對照先依現有標準顯示；後續可繼續補齊。 / Hiện hiển thị các đối chiếu tiêu chuẩn đã có; có thể bổ sung tiếp sau.</div></div>`;
}
function quoteSpecialTable(std){
  const sp=std.special||{},shoe=sp.SHOELACE;
  const shoeRow=shoe?`<tr><td>鞋帶 / Dây giày</td><td>${fmt(shoe.qtyPerTable,0)} 雙 / đôi</td><td>${fmt(shoe.workers,0)} 人 / người</td><td>${fmt(shoe.layHours,2)} H / giờ</td><td>${shoe.temp?'暫定 / Tạm thời':'正式 / Chính thức'}</td></tr>`:'';
  return `<div class="quote-standard-section"><b>特殊排料標準 / Tiêu chuẩn xếp liệu đặc biệt</b><div class="table-scroll"><table class="quote-settings-table quote-special-standard-table"><thead><tr><th>類型<br>Loại</th><th>標準方式<br>Cách tính chuẩn</th><th>固定參數<br>Thông số cố định</th><th>產能依據<br>Cơ sở năng suất</th><th>狀態<br>Trạng thái</th></tr></thead><tbody>${shoeRow}<tr><td>手印單片 / Miếng rời In tay</td><td>依實際排版 / Theo bố trí thực tế</td><td>25M/單邊 × 2邊；每排間隔6MM<br>25M/1 bên × 2 bên; cách hàng 6MM</td><td>層數產能表 / Bảng năng suất số lớp</td><td>正式 / Chính thức</td></tr><tr><td>K3單片 / Miếng rời K3</td><td>44大格 × 每格PC<br>44 ô lớn × PC/ô</td><td>44大格鎖定 / Cố định 44 ô lớn</td><td>層數產能表 / Bảng năng suất số lớp</td><td>正式 / Chính thức</td></tr></tbody></table></div><div class="quote-standard-explain"><b>單片怎麼算 / Cách tính miếng rời</b><br>手印：單邊可用25M，輸入每PC長度與每排（每網版）可排PC；每排之間固定6MM，先算單邊排數，再 × 每排PC × 2邊。 / In tay: 25M dùng được cho 1 bên; nhập độ dài mỗi PC và số PC mỗi hàng (mỗi khuôn); khoảng cách giữa mỗi hàng cố định 6MM, tính số hàng 1 bên rồi × PC/hàng × 2 bên.<br>K3：整桌固定44大格，輸入每大格可排PC；每桌PC = 44 × 每格PC。 / K3: cố định 44 ô lớn; nhập PC mỗi ô; PC/bàn = 44 × PC/ô.<br>兩者8H可完成桌數皆依目前「層數產能」標準換算。 / Số bàn hoàn thành trong 8H của cả hai đều lấy theo tiêu chuẩn năng suất số lớp hiện tại.</div></div>`;
}
...
function quoteSnapshotHtml(std,showHeader=true){
  return `${showHeader?`<div class="quote-version-head"><b>${quoteEscape(std.version)}</b><span>${quoteEscape(std.note)}</span></div>`:''}${quoteLayerTable(std)}<div class="quote-standard-grid">${quoteStripTable('手印 / In tay',std.strips.HAND||[])}${quoteStripTable('機印 / In máy',std.strips.MACHINE||[],true)}${quoteStripTable('K3',std.strips.K3||[])}</div>${quoteSpecialTable(std)}${showHeader?quoteDipHeadQcTable(std):''}${showHeader?quoteDipExperimentTable():''}<div class="quote-standard-section"><b>其他基準 / Tiêu chuẩn khác</b><div class="quote-standard-mini">手印一般長帶桌長 / Chiều dài bàn dây dài In tay：<b>${std.tableLength.HAND}Y</b>｜手印單片 / Miếng rời In tay：<b>25M/單邊，6MM排距，2邊 / 25M mỗi bên, cách 6MM, 2 bên</b>　｜　K3一般長帶 / Dây dài K3：<b>${std.tableLength.K3}Y</b>｜K3單片 / Miếng rời K3：<b>44大格 / 44 ô lớn</b>　｜　一桌 / 1 bàn：<b>${std.sides} 邊 / bên</b>　｜　一般長帶 8H：<b>12H × 8/12</b><br>機印 / In máy：<b>印刷條數依QC標準帶入 / Số dây theo QC｜PC/Y可切換 / chọn PC/Y｜PC需輸入每行圖案數、Y不需輸入 / PC nhập số hình mỗi hàng, Y không cần nhập｜速度 200 / Tốc độ 200</b><br>浸染 / Nhúng sơn：<b>0817 單頭實驗 / dữ liệu 1 đầu｜無前加工用帶身QC；有前加工用束頭QC / không gia công dùng QC thân, có gia công dùng QC đầu｜採下一級較大實驗QC；超過最大實驗QC必須重測 / dùng QC thử nghiệm lớn hơn kế tiếp; vượt QC lớn nhất phải thử lại｜1 PC = 2頭 / 2 đầu｜標準分攤 ${fmt(std.dipAllocHours||4,2)}H（本次可調） / phân bổ ${fmt(std.dipAllocHours||4,2)}H (có thể chỉnh)</b></div></div>`;
}
...
if($('quoteUnitPc')){
  ...
  ['quoteInk','quotePcLength','quoteLayers','quoteTableLength','quotePieceLength','quoteHandPiecePcPerRow','quoteK3PiecePcPerCell','quoteMachinePatterns','quoteMachineLength','quoteMachineStrips','quoteDipHeadSize','quoteDipTimes','quoteDipDepth1','quoteDipDepth2','quoteDipDepth3','quoteDipDepth4','quoteDipDepth5','quoteDipHourPc','quoteDipAllocHours','quoteDipBaseLayers','quoteDipColorLayers','quoteDipTopLayers'].forEach(id=>{if($(id))$(id).addEventListener('input',calcQuoteCapacity);});
...
async function wipExportProcessCapacity(){
  ...
  const heads=['製程\nCông đoạn','已到98-G100未完工\nChưa HT đã tới 98-G100','單位\nĐơn vị','標準產能\nNăng lực chuẩn','標準工時\nGiờ chuẩn','每日運轉\nGiờ chạy/ngày','預估需要天數\nSố ngày cần'];
...
/* 2026-09-18：印刷歷年完工量分析 V4（獨立工具，不改動既有工具）
   V4 修正：
   0) 新增手印／機印／膠片的年度、每月與總筆數統計；年增率算法維持原本不變。
   V3 修正：
...
function phHeaderMap(row){
  const map={};(row||[]).forEach((v,i)=>{const k=phNormHeader(v);if(!k)return;
    ...
    else if(['單位','单位','counit','unit'].includes(k))map.unit=i;
    else if(['完工量','完成量','cofinish','finishqty','qty'].includes(k))map.qty=i;
...
function phLooksLikeHeader(row){const m=phHeaderMap(row);return ['date','width','unit','qty','yard','name','note1','note2'].filter(k=>m[k]!=null).length>=5;}
...
function phFilmPairs(unit,qty){
  ...
  if(qty==null||qty<0)return {ok:false,pairs:null,reason:'膠片完工量無效 / SL hoàn thành đầu keo không hợp lệ'};
  if(u.startsWith('9D'))return {ok:true,pairs:qty,unit:'9D'};
  if(u.startsWith('9C'))return {ok:true,pairs:qty/2,unit:'9C'};
  return {ok:false,pairs:null,reason:`膠片單位 ${u||'(空白)'} 非 9D/9C / Đơn vị đầu keo không phải 9D/9C`};
}
function phGroupText(groups){return groups.map(g=>PH_TARGET_GROUPS[g]).join(' + ');}
function phProcessRow(row,ctx){
  if(!row||!row.length)return;ctx.raw++;
  const m=ctx.map;const get=k=>m[k]!=null?(row[m[k]]??''):'';
  const d=phDate(get('date')),width=phNum(get('width')),yard=phNum(get('yard')),qty=phNum(get('qty')),unit=String(get('unit')??'').trim();
  ...
  const cls=phClassify(types);let reason='';
  ...
  if(!reason&&(cls.groups.includes('HAND')||cls.groups.includes('MACHINE'))){
    ...
    else if(yard==null||yard<0)reason='手印/機印折合碼無效 / Yard quy đổi In tay/In máy không hợp lệ';
    else eq25=yard*width/25;
  }
...
async function phParseCsvStream(file,progressCb){
  ...
  function processText(text){for(let i=0;i<text.length;i++){const ch=text[i];if(ch==='"'){if(inQuotes&&text[i+1]==='"'){field+='"';i++;}else inQuotes=!inQuotes;}else if(ch===','&&!inQuotes){emitField();}else if((ch==='\n'||ch==='\r')&&!inQuotes){if(ch==='\r'&&text[i+1]==='\n')i++;emitField();if(row.some(v=>String(v).trim()!==''))emitRow();}else field+=ch;}}
  while(true){const {done,value}=await reader.read();if(done)break;loaded+=value.byteLength;let text=pending+decoder.decode(value,{stream:true});pending='';if(text.endsWith('"')&&inQuotes){pending='"';text=text.slice(0,-1);}else if(text.endsWith('\r')&&!inQuotes){pending='\r';text=text.slice(0,-1);}processText(text);if(progressCb)progressCb(Math.min(.99,loaded/file.size));}
...
function phAgg(){
  ...
  for(const r of phRows){if(!years.has(r.year))years.set(r.year,phBlankAgg());if(!months.has(r.ym))months.set(r.ym,phBlankAgg());for(const a of [years.get(r.year),months.get(r.ym)]){if(r.groups.includes('HAND')){a.HAND.yard+=r.yard||0;a.HAND.eq+=r.eq25||0;a.HAND.n++;a.totalY+=r.eq25||0;}if(r.groups.includes('MACHINE')){a.MACHINE.yard+=r.yard||0;a.MACHINE.eq+=r.eq25||0;a.MACHINE.n++;a.totalY+=r.eq25||0;}if(r.groups.includes('FILM')){a.FILM.pairs+=r.filmPairs||0;a.FILM.n++;}a.n++;const dt=r.date;if(!a.minDate||dt<a.minDate)a.minDate=dt;if(!a.maxDate||dt>a.maxDate)a.maxDate=dt;}}
  return {years,months};
}
function phTotals(){let hand=0,machine=0,film=0,handN=0,machineN=0,filmN=0;for(const r of phRows){if(r.groups.includes('HAND')){hand+=r.eq25||0;handN++;}if(r.groups.includes('MACHINE')){machine+=r.eq25||0;machineN++;}if(r.groups.includes('FILM')){film+=r.filmPairs||0;filmN++;}}return {hand,machine,film,handN,machineN,filmN,totalY:hand+machine,totalN:handN+machineN};}
function phRender(){
  phStats=phAgg();const ys=[...phStats.years.keys()].sort((a,b)=>a-b),ms=[...phStats.months.keys()].sort(),t=phTotals();$('phValidRows').textContent=phFmt(phRows.length);$('phHandTotal').textContent=phFmt(t.hand,2)+' Y';$('phHandCount').textContent=phFmt(t.handN);$('phMachineTotal').textContent=phFmt(t.machine,2)+' Y';$('phMachineCount').textContent=phFmt(t.machineN);$('phFilmTotal').textContent=phFmt(t.film,2)+' 雙';$('phFilmCount').textContent=phFmt(t.filmN);$('phGrandTotal').textContent=phFmt(t.totalY,2)+' Y';
  let prevY=null,prevFilm=null;$('phYearBody').innerHTML=ys.map(y=>{const a=phStats.years.get(y);let yoy='—',fYoy='—',yc='ph-muted',fc='ph-muted';if(prevY!=null&&prevY>0){const p=(a.totalY-prevY)/prevY*100;yoy=(p>=0?'+':'')+p.toFixed(1)+'%';yc=p>=0?'ph-positive':'ph-negative';}if(prevFilm!=null&&prevFilm>0){const p=(a.FILM.pairs-prevFilm)/prevFilm*100;fYoy=(p>=0?'+':'')+p.toFixed(1)+'%';fc=p>=0?'ph-positive':'ph-negative';}prevY=a.totalY;prevFilm=a.FILM.pairs;return `<tr><td>${y}</td><td>${phFmt(a.HAND.yard,2)}</td><td>${phFmt(a.HAND.eq,2)}</td><td>${phFmt(a.HAND.n)}</td><td>${phFmt(a.MACHINE.yard,2)}</td><td>${phFmt(a.MACHINE.eq,2)}</td><td>${phFmt(a.MACHINE.n)}</td><td><b>${phFmt(a.totalY,2)}</b></td><td><b>${phFmt(a.HAND.n+a.MACHINE.n)}</b></td><td class="${yc}">${yoy}</td><td><b>${phFmt(a.FILM.pairs,2)}</b></td><td>${phFmt(a.FILM.n)}</td><td class="${fc}">${fYoy}</td></tr>`;}).join('');
  $('phMonthBody').innerHTML=ms.map(k=>{const a=phStats.months.get(k);return `<tr><td>${k.replace('-','/')}</td><td>${phFmt(a.HAND.eq,2)}</td><td>${phFmt(a.HAND.n)}</td><td>${phFmt(a.MACHINE.eq,2)}</td><td>${phFmt(a.MACHINE.n)}</td><td><b>${phFmt(a.totalY,2)}</b></td><td><b>${phFmt(a.HAND.n+a.MACHINE.n)}</b></td><td>${phFmt(a.FILM.pairs,2)}</td><td>${phFmt(a.FILM.n)}</td></tr>`;}).join('');phRenderYtd(ys);
}

function phRenderYtd(ys){
  const box=$('phYtdBox');box.classList.add('hidden');if(ys.length<2)return;const latest=ys[ys.length-1],prev=latest-1;if(!phStats.years.has(prev))return;const latestDates=phRows.filter(r=>r.year===latest).map(r=>phDate(r.date)).filter(Boolean);if(!latestDates.length)return;const maxDate=new Date(Math.max(...latestDates.map(d=>d.getTime())));if(maxDate.getMonth()===11&&maxDate.getDate()===31)return;const cm=maxDate.getMonth()+1,cd=maxDate.getDate();const inCutoff=r=>r.month<cm||(r.month===cm&&Number((r.date||'').slice(-2))<=cd);const latestRows=phRows.filter(r=>r.year===latest&&inCutoff(r)),prevRows=phRows.filter(r=>r.year===prev&&inCutoff(r));const sum=a=>{let y=0,f=0;for(const r of a){if(r.groups.includes('HAND'))y+=r.eq25||0;if(r.groups.includes('MACHINE'))y+=r.eq25||0;if(r.groups.includes('FILM'))f+=r.filmPairs||0;}return {y,f};},a=sum(latestRows),b=sum(prevRows),cut=`${String(cm).padStart(2,'0')}/${String(cd).padStart(2,'0')}`;let lines=[];if(b.y>0){const p=(a.y-b.y)/b.y*100;lines.push(`手印＋機印 25MM：${phFmt(b.y,2)} Y → ${phFmt(a.y,2)} Y，<span class="${p>=0?'ph-positive':'ph-negative'}">${p>=0?'+':''}${p.toFixed(1)}%</span>`);}if(b.f>0){const p=(a.f-b.f)/b.f*100;lines.push(`膠片：${phFmt(b.f,2)} 雙 → ${phFmt(a.f,2)} 雙，<span class="${p>=0?'ph-positive':'ph-negative'}">${p>=0?'+':''}${p.toFixed(1)}%</span>`);}if(!lines.length)return;box.innerHTML=`<b>同期比較 / So sánh cùng kỳ：</b>${prev}/01/01～${cut} vs ${latest}/01/01～${cut}<br>${lines.join('<br>')}<br><span class="ph-muted">Y 與雙為不同單位，分開比較，不相加。</span>`;box.classList.remove('hidden');
}
function phStyleSheet(ws,widths){ws.getRow(1).height=42;ws.getRow(1).eachCell(c=>{c.font={bold:true,color:{argb:'FFFFFFFF'}};c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF17365D'}};c.alignment={horizontal:'center',vertical:'middle',wrapText:true};});widths.forEach((w,i)=>ws.getColumn(i+1).width=w);for(let r=2;r<=ws.rowCount;r++)ws.getRow(r).eachCell((c,i)=>{c.alignment={vertical:'middle',horizontal:i===1?'left':'right'};if(typeof c.value==='number')c.numFmt='#,##0.00';});}
async function phExport(){
  if(!phRows.length||!phStats)return;const wb=new ExcelJS.Workbook();wb.creator='工廠工具箱';const ys=[...phStats.years.keys()].sort((a,b)=>a-b),ms=[...phStats.months.keys()].sort();
  const yws=wb.addWorksheet('年度統計 Thống kê năm',{views:[{state:'frozen',ySplit:1}]});yws.addRow(['年度\nNăm','手印原折合碼\nYard gốc In tay','手印25MM等效\n25MM In tay','手印筆數\nSố dòng In tay','機印原折合碼\nYard gốc In máy','機印25MM等效\n25MM In máy','機印筆數\nSố dòng In máy','手印+機印25MM合計\nTổng 25MM','手印+機印筆數\nTổng số dòng IN','Y年增率\nTăng/giảm Y','膠片完工\nĐầu keo (雙/Đôi)','膠片筆數\nSố dòng đầu keo','膠片年增率\nTăng/giảm']);let py=null,pf=null;ys.forEach(y=>{const a=phStats.years.get(y),yr=py&&py>0?(a.totalY-py)/py:null,fr=pf&&pf>0?(a.FILM.pairs-pf)/pf:null;yws.addRow([y,a.HAND.yard,a.HAND.eq,a.HAND.n,a.MACHINE.yard,a.MACHINE.eq,a.MACHINE.n,a.totalY,a.HAND.n+a.MACHINE.n,yr,a.FILM.pairs,a.FILM.n,fr]);py=a.totalY;pf=a.FILM.pairs;});phStyleSheet(yws,[12,22,22,18,22,22,18,24,20,16,22,18,16]);for(let r=2;r<=yws.rowCount;r++){yws.getCell(r,10).numFmt='0.0%';yws.getCell(r,13).numFmt='0.0%';}
  const mws=wb.addWorksheet('每月統計 Thống kê tháng',{views:[{state:'frozen',ySplit:1}]});mws.addRow(['年月\nTháng','手印25MM等效\nIn tay (Y)','手印筆數\nSố dòng In tay','機印25MM等效\nIn máy (Y)','機印筆數\nSố dòng In máy','手印+機印25MM合計\nTổng (Y)','手印+機印筆數\nTổng số dòng IN','膠片完工\nĐầu keo (雙/Đôi)','膠片筆數\nSố dòng đầu keo']);ms.forEach(k=>{const a=phStats.months.get(k);mws.addRow([k,a.HAND.eq,a.HAND.n,a.MACHINE.eq,a.MACHINE.n,a.totalY,a.HAND.n+a.MACHINE.n,a.FILM.pairs,a.FILM.n]);});phStyleSheet(mws,[14,24,18,24,18,25,20,24,18]);
  ...
  const buf=await wb.xlsx.writeBuffer(),blob=new Blob([buf],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='印刷-歷年完工量分析_V4_25MM_膠片雙數_筆數.xlsx';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),2000);
}
