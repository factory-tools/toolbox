...
function parseBatch(){
  ...
  if(!text){$('printingStatus').className='status warn';$('printingStatus').textContent='請先從 Excel 複製 5 欄資料並貼到黃色區域 / Vui lòng sao chép 5 cột từ Excel và dán vào ô màu vàng';return;}
  ...
  text.split(/\r?\n/).forEach((line,idx)=>{
    ...
    parsed.push({qty,unit,width,length:length>0?length:'',method:method==='K3'?'K3':'HAND'});
  });
  if(!parsed.length){$('printingStatus').className='status warn';$('printingStatus').textContent='無法匯入。請依序貼上：訂單量、單位、寬度mm、長度mm、印刷方式 / Không thể nhập. Thứ tự: SL, đơn vị, khổ mm, dài mm, phương pháp';return;}
  $('orderBody').innerHTML='';
...
addOrder({qty:'',unit:'PC',width:25,length:'',method:'HAND'});
