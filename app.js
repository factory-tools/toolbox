...
const QUOTE_STANDARD_HISTORY=[
  {
    version:'2026/08/25',
    note:'更新手印/K3、機印，新增浸染報價（0817單頭實驗、1PC=2頭、每批報廢10KG、4H分攤） / Cập nhật In tay/K3, In máy; thêm báo giá In nhúng theo dữ liệu thử 0817 (1 PC=2 đầu)',
    layers:{
...
function quoteDipCalc(){
  ...
  const cap4=hourPc>0?hourPc*QUOTE_DIP_ALLOC_HOURS:0;
  ...
  $('quoteWhatYouAreQuoting').textContent=`目前：浸染｜${type}｜${ink}｜${colors}色｜PC / Hiện tại: In nhúng｜${type}｜${ink}｜${colors} màu｜PC`;
...
function quoteMethodName(v){return v==='DIP'?'浸染 / In nhúng':v==='MACHINE'?'機印 / In máy':v==='K3'?'K3':'手印 / In tay';}
...
function updateQuoteLayoutUI(){
  ...
  if(isSpecial&&sp){
    ...
    $('quoteSpecialLayHours').textContent=`${fmt(sp.layHours,2)} H / giờ`;
  }
  if(!isSpecial){
    $('quotePcLengthField').classList.toggle('hidden',quoteUnit!=='PC');
    if($('quoteUnitToggle'))$('quoteUnitToggle').classList.remove('hidden');
  }
  calcQuoteCapacity();
}
function calcQuoteCapacity(){
  ...
  if(layout!=='NORMAL'){
    ...
    $('quoteTables12').textContent=printHours>0?`${fmt(printHours,2)} H / 桌印刷 / giờ in mỗi bàn`:'—';
    $('quoteTables8').textContent=totalHours>0?`${fmt(8/totalHours,2)} 桌 / bàn`:'—';
    $('quoteTotalHours').textContent=totalHours>0?`${fmt(totalHours,2)} H / giờ（${fmt(layHours,2)}+${fmt(printHours,2)}）`:'—';
    ...
    else $('quoteCapacity8Alt').textContent=`排料 ${fmt(workers,0)}人 × ${fmt(layHours,2)}H = ${fmt(workers*layHours,0)} 人時/桌 / Xếp liệu ${fmt(workers,0)} người × ${fmt(layHours,2)}H = ${fmt(workers*layHours,0)} giờ-người/bàn`;
...
if($('quoteUnitPc')){
  ...
  setQuoteMachineStroke(1);quoteSetUnit('PC');updateQuoteTableLength();updateQuoteLayoutUI();renderQuoteStandards();
}
...
async function wipExportCurrent(){
  if(!wipFiltered.length)return;const addedHeads=['MSK','MSK類型 / Loại MSK','涉及製程 / Công đoạn','98-G100狀態 / Trạng thái','原單位未完工 / Chưa HT đơn vị gốc','未完工折合碼 / Yard chưa HT','25MM等效未完工 / Quy đổi 25MM','膠片未完工雙 / Đôi đầu keo chưa HT','折合碼狀態 / Trạng thái quy đổi','CO_KEYDATE','30天期限 / Hạn 30 ngày','逾期天數 / Số ngày trễ','是否超30天 / Quá 30 ngày?','判斷 / Kết quả','來源內容 / Nội dung nguồn'];const heads=[...wipSourceHeaders,...addedHeads];
  ...
  if(typeof ExcelJS==='undefined'){const ws=XLSX.utils.aoa_to_sheet([heads,...data]);ws['!autofilter']={ref:XLSX.utils.encode_range({r:0,c:0},{r:data.length,c:heads.length-1})};const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'明細 Chi tiết');XLSX.writeFile(wb,'印刷-未完工產能分析_IN_目前明細.xlsx');return;}
  ...
  stat.addRow(['製程 / Công đoạn','筆數 / Số nét','已到98-G100未完工 / Chưa HT đã tới 98-G100','未到98-G100未完工 / Chưa HT chưa tới 98-G100','總未完工 / Tổng chưa HT','單位 / Đơn vị']);
  ...
  const overdue=wb.addWorksheet('超30天 Quá 30 ngày',{views:[{state:'frozen',ySplit:1}]});overdue.addRow(['製程 / Công đoạn','超30天筆數 / Số nét quá 30 ngày','超30天未完工 / Chưa HT quá 30 ngày','單位 / Đơn vị']);[['手印總量 / Tổng In tay',['HAND','HAND_TRANSFER'],'Y'],['機印總量 / Tổng In máy',['MACHINE','MACHINE_TRANSFER'],'Y'],['移印 / In chấm',['PAD'],'Y'],['噴塗 / Phun silicon',['SPRAY'],'Y'],['膠片 / Đầu keo',['FILM'],'雙 / Đôi']].forEach(([n,gs,u])=>{const base=wipFiltered.filter(r=>r.over30),a=base.filter(r=>gs.some(g=>wipHasGroup(r,g)));let v=u==='雙'?wipSum(a,'pairUnfinished'):gs.reduce((s,g)=>s+wipSum(wipRowsForGroup(base,g),'eq25'),0);overdue.addRow([n,a.length,v,u]);});wipStyleSummarySheet(overdue,[28,18,24,14]);
